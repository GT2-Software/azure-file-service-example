...
public class LogToBlobService {

    private static Logger logger = null;
    private static LoggerContext context = new LoggerContext("LogBlobContext");
    private static final BlobService blobService = new BlobService();

    private LogToBlobService() {
    }
    ...
    public static Logger getInstance() {
        if (logger == null) {
            try {
                ...
                context = new LoggerContext("LogBlobContext");
    ...
    public static void error(String Message) {
        ...
        if (System.getenv("LOG_ERROR").equals("1")) {
            ...
            blobService.WriteToBlobTextFile(
                    String.format("%s/%s/%s/%s",
                            System.getenv("LOG_ROOT_PATH_BLOB"),
                            System.getenv("COUNTRY"),
                            System.getenv("COMPUTERNAME"),
                            "log.log"),
                    "test-container-crud",
                    String.format("%s - %s - %s", LogToBlobService.class, "ERROR", Message));
        }
    }
        ...
        if (System.getenv("LOG_INFO").equals("1")) {
            ...
            blobService.WriteToBlobTextFile(
                    String.format("%s/%s/%s/%s",
                            ...
                            "log.log"),
                    "test-container-crud",
                    String.format("%s - %s - %s", LogToBlobService.class, "INFO", Message));
        }
    }
